...
import { playtimeBotRuntime, syncPlaytimeBots, restartPlaytimeBot, stopPlaytimeBot, shutdownPlaytimeBots, testPlaytimeWardogs, refreshPlaytimeTracker, playtimeTrackerSnapshot, playtimeTrackerServers, combinedStatsSnapshot, searchCombinedPlayers, leaderboardPositions, PLAYTIME_SERVICE_ID } from './playtime-tracker.js';
...
function playtimeTrackerForm(req,service,bot) {
  ...
  return `<form method="post" action="/bot-services/${encodeURIComponent(service.id)}/manage" class="panel formgrid" id="playtime-tracker-config">
    ...
    <div class="span2 help"><strong>WARDOGS Status Bot:</strong> ${tr(lang,'Jeder eingetragene Gameserver hat seinen eigenen festen Discord-Killfeed mit den letzten 15 Kills. Discord↔Game-Verknüpfung per /linkgame, /status, /whois, /whoisplayer und /playerstats laufen über denselben Bot. /leaderboard zeigt Top 15 pro Kategorie mit Spielersuche und exakter Platzierung; optional kann dieselbe Topliste als feste Nachricht in einem Channel stehen.','Every configured game server has its own fixed Discord killfeed with the latest 15 kills. Discord↔game linking via /linkgame, /status, /whois, /whoisplayer and /playerstats run through the same bot. /leaderboard shows the Top 15 per category with player search and exact ranking; optionally the same leaderboard can stay as a fixed channel message.')}</div>
...
function playtimeStatsPanel(req,bot) {
  ...
  const leaderboardCards=(combined.categories||[]).map((category)=>{
    const rows=category.rows.slice(0,15).map((p,i)=>`<tr><td class="leaderboard-rank">${medals[i]||`${i+1}.`}</td><td><a class="leaderboard-player" href="${manageBase}&playerid=${encodeURIComponent(p.steamId)}#leaderboard-player"><strong>${esc(p.name||p.steamId)}</strong></a><div class="muted small">${esc(p.steamId)}</div></td><td class="leaderboard-value"><strong>${esc(category.format(p))}</strong></td></tr>`).join('');
    return `<article class="panel tablewrap leaderboard-card"><div class="managed-heading padded"><div><span class="eyebrow">Top 15</span><h3>${esc(category.label)}</h3></div></div><table><thead><tr><th>#</th><th>${l(req,'Spieler','Player')}</th><th>${l(req,'Wert','Value')}</th></tr></thead><tbody>${rows||`<tr><td colspan="3">${l(req,'Noch keine Daten erfasst.','No data tracked yet.')}</td></tr>`}</tbody></table></article>`;
  }).join('');
  ...
  const playerDetail=selectedPlayer?`<section class="panel leaderboard-player-detail" id="leaderboard-player"><div class="row between wrap"><div><span class="eyebrow">${l(req,'Spielerprofil & Leaderboard-Position','Player profile & leaderboard position')}</span><h2>${esc(selectedPlayer.name||selectedPlayer.steamId)}</h2><p class="muted small"><a href="https://steamcommunity.com/profiles/${esc(selectedPlayer.steamId)}" target="_blank" rel="noopener">${esc(selectedPlayer.steamId)}</a>${discordLink?` · Discord ID ${esc(discordLink.discordUserId)}`:''}${selectedPlayer.lastSeenAt?` · ${l(req,'zuletzt gesehen','last seen')} ${esc(new Date(selectedPlayer.lastSeenAt).toLocaleString(localeCode(langOf(req))))}`:''}</p></div><a class="button ghost smallbtn" href="${manageBase}#alltime-leaderboard">${l(req,'Auswahl schließen','Close selection')}</a></div><section class="leaderboard-player-stats"><article><span>Kills</span><strong>${esc(selectedPlayer.kills)}</strong></article><article><span>${l(req,'Tode','Deaths')}</span><strong>${esc(selectedPlayer.deaths)}</strong></article><article><span>K/D</span><strong>${esc(Number(selectedPlayer.kd||0).toLocaleString('de-DE',{minimumFractionDigits:2,maximumFractionDigits:2}))}</strong></article><article><span>${l(req,'Spielzeit','Playtime')}</span><strong>${esc(trackedDuration(selectedPlayer.totalSeconds))}</strong></article><article><span>${l(req,'Kill-Rekord / Match','Kill record / match')}</span><strong>${esc(selectedPlayer.killRecord||0)}</strong></article><article><span>Headshot-Rate</span><strong>${esc((Number(selectedPlayer.headshotRate||0)*100).toLocaleString('de-DE',{minimumFractionDigits:1,maximumFractionDigits:1}))}%</strong></article><article><span>${l(req,'Match-Siege','Match wins')}</span><strong>${esc(selectedPlayer.matchWins||0)} (${esc((Number(selectedPlayer.winRate||0)*100).toLocaleString('de-DE',{maximumFractionDigits:0}))}%)</strong></article><article><span>${l(req,'Matches gespielt','Matches played')}</span><strong>${esc(selectedPlayer.matchesPlayed||0)}</strong></article><article><span>${l(req,'Seeding-Zeit (1–20 Spieler)','Seeding time (1–20 players)')}</span><strong>${esc(trackedDuration(selectedPlayer.seedingSeconds))}</strong></article></section><div class="chips leaderboard-ranks">${rankChips}</div></section>`:'';
  ...
  return `<section class="playtime-dashboard">
    ...
    <section id="alltime-leaderboard" class="leaderboard-section"><div class="managed-heading padded"><div><span class="eyebrow">🏆 All-Time Leaderboard</span><h2>${l(req,'Top 15 pro Kategorie','Top 15 per category')}</h2><p class="muted small">${l(req,'K/D wird wie im Beispiel erst ab 100 Kills gewertet. Die Platzierung darunter funktioniert auch außerhalb der Top 15.','As in the example, K/D is ranked only from 100 kills. The search below shows exact placement even outside the Top 15.')}</p></div></div><div class="leaderboard-grid">${leaderboardCards}</div></section>
