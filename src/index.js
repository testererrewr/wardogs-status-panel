...
function render(req, res, title, body) { res.send(layout({ title, body, user: currentUser(req), csrf: csrf(req), flash: takeFlash(req), lang: langOf(req) })); }
...
app.post('/get-more/verify', requireLogin, checkCsrf, async (req, res) => {
  try { const result = await verifyFreeBoostForUser(currentUser(req).discordId); rebalanceAssignments(); flash(req,'ok',result.state==='verified'?l(req,`Free-Boost verifiziert: ${result.freeBotLimit} Bots gesamt.`,`Free boost verified: ${result.freeBotLimit} bots total.`):l(req,'Branding-Channel wurde nicht korrekt ganz oben gefunden.','Branding channel was not found correctly at the very top.')); }
...
app.get('/donate', (req, res) => {
  ...
  render(req,res,tr(lang,'Spenden','Donate'),`<div class="donate-hero panel"><span class="eyebrow">status-hub.lol</span><h1>${tr(lang,'Server Status Hub unterstützen','Support Server Status Hub')}</h1><p>${tr(lang,'Spenden helfen bei VPS-Kosten, Entwicklung und mehr kostenlosen Kapazitäten.','Donations help cover VPS costs, development and more free capacity.')}</p><div class="actions wrap">${buttons || `<span class="muted">${tr(lang,'Spendenlinks werden noch eingerichtet.','Donation links are being set up.')}</span>`}</div></div><div class="pagehead supporters-head"><div><h2>${tr(lang,'Supporter','Supporters')}</h2><p>${tr(lang,'Danke an alle, die das Projekt unterstützen.','Thank you to everyone supporting the project.')}</p></div></div><section class="supporter-grid">${wall || `<div class="empty panel">${tr(lang,'Noch keine öffentlichen Supporter eingetragen.','No public supporters listed yet.')}</div>`}</section>`);
});

app.get('/', requireLogin, (req, res) => {
  const lang = langOf(req); const u = currentUser(req); const db = readDb(); const settings = db.siteSettings || getSiteSettings(); const servers = listServersFor(u.discordId, u.role === 'admin');
...
app.post('/servers/:id/restart', requireLogin, checkCsrf, async (req, res) => { const s = ownedServer(req, req.params.id); if (!s) return res.status(404).send('Nicht gefunden'); upsertServer({ id: s.id, restartNonce: Date.now() }); rebalanceAssignments(); flash(req,'ok','Bot-Neustart an den Status-Node weitergegeben.'); res.redirect('/'); });
...
app.get('/custom-bots', requireLogin, async (req, res) => {
  const lang=langOf(req); const u=currentUser(req); const bots=listCustomBotsFor(u.discordId,u.role==='admin');
...
app.get('/custom-bots/new', requireLogin, (req,res)=>{
  ...
  if(count>=customLimit(u))return res.status(403).send(l(req,'Keine freien Custom-Bot-Slots.','No free custom bot slots.'));
