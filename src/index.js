...
app.post('/managed-bots/:id/ignore/add', requireLogin, checkCsrf, async(req,res)=>{
  ...
  const steamId=normalizeSteamId64(req.body.steamId);if(!steamId){flash(req,'err',l(req,'Ungültige SteamID64.','Invalid SteamID64.'));return res.redirect(`/bot-services/${encodeURIComponent(bot.serviceId)}/manage`);}
  ...
  await syncManagedBots(readDb().managedBots||[]);flash(req,'ok',l(req,'Spieler wird ab jetzt von Detection-Warnungen und Auto-Ban ignoriert.','Player is now ignored by detection alerts and auto-ban.'));res.redirect(`/bot-services/${encodeURIComponent(bot.serviceId)}/manage`);
});
...
function managedActionContext(req,res){
  ...
  if(!bot.wardogsBaseUrl||!bot.wardogsSecretEnc){flash(req,'err',l(req,'WARDOGS URL und RCON-Passwort müssen zuerst gespeichert werden.','WARDOGS URL and RCON password must be saved first.'));res.redirect(`/bot-services/${encodeURIComponent(bot.serviceId)}/manage`);return null;}
  return bot;
}
function managedBack(res,bot){return res.redirect(`/bot-services/${encodeURIComponent(bot.serviceId)}/manage`);}
