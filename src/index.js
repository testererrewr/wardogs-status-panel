...
import { managedBotRuntime, parseManagedRules, testManagedWardogs, syncManagedBots, restartManagedBot, stopManagedBot, shutdownManagedBots, managedDashboard, managedMapOptions, MANAGED_DISCORD_PERMISSION_KEYS, broadcastManaged, banManagedPlayer, temporaryBanManagedPlayer, kickManagedPlayer, killManagedPlayer, whisperManagedPlayer, moveManagedPlayer, unbanManagedPlayer, addManagedReservedSlot, removeManagedReservedSlot, restartManagedMatch, endManagedMatch, setManagedLighting, changeManagedMap, normalizeSteamId64, normalizeManagedBanDiscordLink, formatManagedBanDuration } from './managed-bots.js';
...
function managedOperationsPanel(req, bot, live) {
  ...
  const templateOptions=`<option value="">${l(req,'Benutzerdefiniert','Custom')}</option>${templates.map((tpl)=>`<option value="${esc(tpl.id)}">${esc(tpl.label)} · ${tpl.durationMinutes?esc(formatManagedBanDuration(tpl.durationMinutes)):l(req,'permanent','permanent')}</option>`).join('')}`;
  const playerRows=players.map((p)=>{
    ...
    const actionBase=valid?`<div class="managed-player-actions">
      ...
      <form method="post" action="/managed-bots/${esc(bot.id)}/player/ban" onsubmit="return confirm('${l(req,'Ban wirklich ausführen?','Really apply this ban?')}')"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="steamId" value="${esc(steam)}">${templates.length?`<select name="templateId">${templateOptions}</select>`:''}<input name="reason" maxlength="180" placeholder="${l(req,'Ban-Grund','Ban reason')}">${managedDurationControlsHtml(req,'duration',0)}<button class="button danger smallbtn">Ban</button></form>
