...
function managedBotForm(req, service, bot) {
  ...
  const banTemplateRows=managedBanTemplateRowsHtml(req,bot);
   return `<form method="post" action="/bot-services/${encodeURIComponent(service.id)}/manage" class="panel formgrid" id="managed-bot-config">
    ...
    <label class="span2">${tr(lang,'Welcome-Whisper','Welcome whisper')}<textarea name="welcomeWhisperMessage" rows="3" maxlength="200" placeholder="Hello {player}, welcome to the server! Join our Discord: discord.gg/example">${esc(bot.welcomeWhisperMessage||'Hello {player}, welcome to the server! Join our Discord.')}</textarea><span class="muted small">${tr(lang,'Variablen: {player}, {steamid}, {faction}. Versand nach dem Join, sobald WARDOGS ein Team/eine Fraktion meldet (Spawn-Bereitschaft) – auch während Seeding / Waiting for Players.','Variables: {player}, {steamid}, {faction}. Sent after join as soon as WARDOGS reports a team/faction (spawn readiness) – including during seeding / Waiting for Players.')}</span></label>
