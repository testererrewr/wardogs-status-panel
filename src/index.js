...
function managedBotForm(req, service, bot) {
  ...
  const syncTarget=bot.banSyncTargetBotId?getManagedBot(String(bot.banSyncTargetBotId)):null;
  const syncAccepted=Boolean(syncTarget&&Array.isArray(syncTarget.banSyncAcceptedSources)&&syncTarget.banSyncAcceptedSources.map(String).includes(String(bot.id)));
  const syncStatus=bot.banSyncTargetBotId?(syncAccepted?tr(lang,'angenommen / aktiv','accepted / active'):syncTarget?tr(lang,'Anfrage ausstehend','request pending'):tr(lang,'Ziel nicht gefunden','target not found')):tr(lang,'aus','off');
   return `<form method="post" action="/bot-services/${encodeURIComponent(service.id)}/manage" class="panel formgrid" id="managed-bot-config">
    ...
    <div class="span2 managed-config-block"><strong>Dynamic Ban</strong><label class="check"><input type="checkbox" name="dynamicBanEnabled" value="1" ${bot.dynamicBanEnabled===true?'checked':''}> <strong>${tr(lang,'Temporäre Bans als Dynamic Ban ausführen','Use Dynamic Ban for temporary bans')}</strong></label><p class="muted small">${tr(lang,'Dynamic Ban setzt zunächst keinen Game-/WARDOGS-Ban. Der Spieler wird bei jedem erneuten Join gekickt, solange die eingestellte Ban-Zeit läuft. Rejoint derselbe Spieler zu oft im eingestellten Zeitfenster, wird er für den REST der ursprünglichen Laufzeit in einen normalen WARDOGS-Ban eskaliert. Die Ablaufzeit wird dabei niemals verlängert. Das ist nützlich, weil ein normaler Game-Ban beim erneuten Connect nicht immer eine verständliche Ban-Nachricht zeigt.','Dynamic Ban initially creates no game/WARDOGS ban. The player is kicked on every rejoin while the configured ban duration is active. If the same player rejoins too often inside the configured window, the ban escalates to a normal WARDOGS ban only for the REMAINDER of the original duration. The expiry is never extended. This is useful because a normal game ban does not always show a useful ban message on reconnect.')}</p><div class="managed-add-row"><label>${tr(lang,'Eskalation nach Joins','Escalate after joins')}<input name="dynamicBanEscalateJoins" type="number" min="2" max="20" value="${esc(bot.dynamicBanEscalateJoins||3)}"></label><label>${tr(lang,'Zeitfenster (Minuten)','Window (minutes)')}<input name="dynamicBanEscalateWindowMinutes" type="number" min="1" max="1440" value="${esc(bot.dynamicBanEscalateWindowMinutes||5)}"></label></div></div>
    <div class="span2 managed-config-block"><strong>Ban Sync</strong><p class="muted small">${tr(lang,'Trage die BOT ID des Management-Bots ein, der die Bans dieses Bots übernehmen soll. Der Ziel-Bot bekommt im eigenen Panel eine Anfrage. Erst nach Annahme werden bestehende und neue Bans gespiegelt.','Enter the BOT ID of the management bot that should receive bans from this bot. The target bot gets a request in its own panel. Existing and new bans are mirrored only after the target owner accepts.')}</p><label>${tr(lang,'Aktuelle BOT ID','Current BOT ID')}<input type="text" readonly value="${esc(bot.id)}" onclick="this.select()" aria-label="${tr(lang,'Aktuelle BOT ID','Current BOT ID')}"><span class="muted small">${tr(lang,'Diese ID beim anderen Management-Bot als Ziel BOT ID eintragen.','Enter this ID as the Target BOT ID on the other management bot.')}</span></label><label>${tr(lang,'Ziel BOT ID','Target BOT ID')}<input name="banSyncTargetBotId" maxlength="80" value="${esc(bot.banSyncTargetBotId||'')}" placeholder="${esc(bot.id)}"></label><div class="muted small">Status: <strong>${esc(syncStatus)}</strong>${syncTarget?` · ${esc(syncTarget.name||syncTarget.id)}`:''}</div>${syncAccepted?`<div class="actions"><button class="button ghost smallbtn" type="submit" formaction="/managed-bots/${esc(bot.id)}/ban-sync/resync">${tr(lang,'Jetzt neu synchronisieren','Resync now')}</button></div>`:''}</div>
  ...
  <script src="/managed.js?v=3.12.33" defer></script>`;
}
...
function managedBanSyncPanel(req, bot) {
  const token=esc(csrf(req));
  const pending=(Array.isArray(bot.banSyncRequests)?bot.banSyncRequests:[]).filter((row)=>String(row?.sourceBotId||''));
  const accepted=(Array.isArray(bot.banSyncAcceptedSources)?bot.banSyncAcceptedSources:[]).map(String).filter(Boolean);
  if(!pending.length&&!accepted.length&&!bot.banSyncTargetBotId)return '';
  const pendingRows=pending.map((row)=>{const source=getManagedBot(String(row.sourceBotId||''));return `<tr><td><strong>${esc(source?.name||row.sourceBotId)}</strong><div class="muted small">${esc(row.sourceBotId)}${row.sourceOwnerDiscordId?` · Owner ${esc(row.sourceOwnerDiscordId)}`:''}</div></td><td>${row.requestedAt?esc(new Date(row.requestedAt).toLocaleString(localeCode(langOf(req)))):'—'}</td><td><div class="actions wrap"><form method="post" action="/managed-bots/${esc(bot.id)}/ban-sync/${esc(row.sourceBotId)}/accept"><input type="hidden" name="_csrf" value="${token}"><button class="button success smallbtn">${l(req,'Annehmen','Accept')}</button></form><form method="post" action="/managed-bots/${esc(bot.id)}/ban-sync/${esc(row.sourceBotId)}/reject"><input type="hidden" name="_csrf" value="${token}"><button class="button danger smallbtn">${l(req,'Ablehnen','Reject')}</button></form></div></td></tr>`;}).join('');
  const acceptedRows=accepted.map((sourceId)=>{const source=getManagedBot(sourceId);return `<tr><td><strong>${esc(source?.name||sourceId)}</strong><div class="muted small">${esc(sourceId)}</div></td><td><span class="badge online">${l(req,'aktiv','active')}</span></td><td><form method="post" action="/managed-bots/${esc(bot.id)}/ban-sync/${esc(sourceId)}/reject" onsubmit="return confirm('${l(req,'Ban-Sync wirklich trennen?','Disconnect this ban sync?')}')"><input type="hidden" name="_csrf" value="${token}"><button class="button ghost smallbtn">${l(req,'Trennen','Disconnect')}</button></form></td></tr>`;}).join('');
  return `<div class="panel tablewrap managed-section"><div class="managed-heading padded"><div><span class="eyebrow">Ban Sync</span><h2>${l(req,'Ban-Sync Anfragen','Ban sync requests')}</h2><p>${l(req,'Nur nach ausdrücklicher Annahme darf ein anderer Management-Bot Bans auf diese Instanz spiegeln.','Another management bot may mirror bans to this instance only after explicit acceptance.')}</p></div></div>${pendingRows?`<table><thead><tr><th>${l(req,'Anfragender Bot','Requesting bot')}</th><th>${l(req,'Angefragt','Requested')}</th><th></th></tr></thead><tbody>${pendingRows}</tbody></table>`:''}${acceptedRows?`<div class="padded"><strong>${l(req,'Angenommene Quellen','Accepted sources')}</strong></div><table><thead><tr><th>Bot</th><th>Status</th><th></th></tr></thead><tbody>${acceptedRows}</tbody></table>`:''}</div>`;
}
...
app.post('/bot-services/:id/instances/:botId/delete', requireLogin, checkCsrf, rateLimit({windowMs:60_000,limit:5}), async(req,res)=>{
  ...
  try{
    ...
    if(service.id==='wardogs-warning-bot'){
      // Clean both classic temporary bans and Dynamic Bans. Escalated Dynamic
      // Bans are real WARDOGS bans and must not survive deleting the service.
      ...
      for(const steamId of cleanupIds){try{await unbanManagedPlayer(bot,steamId,{actor:`web:${user.discordId}`,skipSync:false,tolerateMissing:true});}catch{}}
    }
...
app.post('/bot-services/:id/manage', requireLogin, checkCsrf, async(req,res)=>{
  ...
  try{
    ...
    const dynamicBanEnabled=req.body.dynamicBanEnabled==='1';
    const dynamicBanEscalateJoins=Math.max(2,Math.min(20,Math.floor(Number(req.body.dynamicBanEscalateJoins)||3)));
    const dynamicBanEscalateWindowMinutes=Math.max(1,Math.min(1440,Math.floor(Number(req.body.dynamicBanEscalateWindowMinutes)||5)));
    const banSyncTargetBotId=String(req.body.banSyncTargetBotId||'').trim().slice(0,80);
    if(banSyncTargetBotId){const target=getManagedBot(banSyncTargetBotId);if(!target||target.serviceId!=='wardogs-warning-bot')throw new Error(l(req,'Ban-Sync Ziel BOT ID wurde nicht gefunden oder ist kein WARDOGS Management Bot.','Ban-sync target BOT ID was not found or is not a WARDOGS management bot.'));if(target.id===bot.id)throw new Error(l(req,'Ein Bot kann seine Bans nicht mit sich selbst synchronisieren.','A bot cannot sync bans to itself.'));}
    ...
    await requestManagedBanSync(bot,banSyncTargetBotId,`web:${user.discordId}`);
    appendManagedAudit(bot.id,{actor:`web:${user.discordId}`,action:'config-saved',detail:`Dynamic Ban: ${dynamicBanEnabled?'on':'off'} · Ban Sync: ${banSyncTargetBotId||'off'}`});
    await syncAllServiceBots();
...
app.post('/managed-bots/:id/restart', requireLogin, checkCsrf, async(req,res)=>{const bot=ownedManaged(req,req.params.id);if(!bot)return res.status(404).send('Not found');const fromAdmin=isAdmin(req)&&String(req.get('referer')||'').includes('/admin'),wasEnabled=bot.enabled===true;if(!managedAccessActive(bot)&&!isAdmin(req))return res.status(403).send('Access expired');try{const fresh=upsertManagedBot({id:bot.id,enabled:true,restartNonce:Date.now()});rebalanceAssignments();await restartServiceBot(fresh);appendManagedAudit(bot.id,{actor:`web:${currentUser(req)?.discordId||'unknown'}`,action:wasEnabled?'bot-restart':'bot-start'});flash(req,'ok',wasEnabled?l(req,'Managed Bot neu gestartet.','Managed bot restarted.'):l(req,'Managed Bot gestartet.','Managed bot started.'));}catch(error){const latest=getManagedBot(bot.id)||bot;if(latest.autoRecoveryEnabled===false)upsertManagedBot({id:bot.id,enabled:false});appendManagedAudit(bot.id,{actor:`web:${currentUser(req)?.discordId||'unknown'}`,action:wasEnabled?'bot-restart-failed':'bot-start-failed',detail:String(error.message||error).slice(0,300),status:'error'});flash(req,'err',latest.autoRecoveryEnabled===false?error.message:l(req,`${error.message} · Auto-Recovery versucht den Start automatisch erneut.`,`${error.message} · Auto recovery will retry automatically.`));}res.redirect(fromAdmin?'/admin?tab=bots#bots':managedManageUrl(bot.serviceId,bot.id));});
...
function managedConfigPayload(bot){
  ...
  return {...base,alertChannelId:String(bot.alertChannelId||''),mentionRoleId:String(bot.mentionRoleId||''),controlPanelEnabled:bot.controlPanelEnabled===true,controlPanelChannelId:String(bot.controlPanelChannelId||''),discordGrants:managedGrantRows(bot),banTemplates:managedBanTemplates(bot),banDiscordLink:String(bot.banDiscordLink||''),dynamicBanEnabled:bot.dynamicBanEnabled===true,dynamicBanEscalateJoins:Number(bot.dynamicBanEscalateJoins||3),dynamicBanEscalateWindowMinutes:Number(bot.dynamicBanEscalateWindowMinutes||5),banSyncTargetBotId:String(bot.banSyncTargetBotId||''),rulesText:String(bot.rulesText||''),autoBanEnabled:bot.autoBanEnabled===true,announcementEnabled:bot.announcementEnabled===true,announcementIntervalMinutes:Number(bot.announcementIntervalMinutes||15),announcementMessages:String(bot.announcementMessages||''),welcomeWhisperEnabled:bot.welcomeWhisperEnabled===true,welcomeWhisperMessage:String(bot.welcomeWhisperMessage||'')};
}
...
app.post('/managed-bots/:id/config/import',requireLogin,managedConfigFile,checkCsrf,async(req,res)=>{
  ...
  try{
    ...
    if(bot.serviceId===PLAYTIME_SERVICE_ID){
      ...
      const syncTarget=String(data.banSyncTargetBotId||'').trim().slice(0,80);if(syncTarget){const target=getManagedBot(syncTarget);if(!target||target.serviceId!=='wardogs-warning-bot'||target.id===bot.id)throw new Error(l(req,'Ban-Sync Ziel aus der Config ist ungültig.','Ban-sync target in config is invalid.'));}patch.banSyncTargetBotId=syncTarget;
    ...
    const imported=upsertManagedBot(patch);if(bot.serviceId!=='wardogs-playtime-tracker')await requestManagedBanSync(imported,String(patch.banSyncTargetBotId||''),`web:${currentUser(req)?.discordId||'unknown'}`);appendManagedAudit(bot.id,{actor:`web:${currentUser(req)?.discordId||'unknown'}`,action:'config-imported'});await syncAllServiceBots();flash(req,'ok',l(req,'Config importiert. Tokens, Passwörter, Abo-/Besitzdaten und Laufzeitstatistiken wurden nicht überschrieben.','Config imported. Tokens, passwords, subscription/ownership data and runtime statistics were not overwritten.'));
