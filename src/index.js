...
app.get('/servers/new', requireLogin, (req, res) => {
  ...
  if (count >= statusLimit(u)) { flash(req,'limit',l(req,'Dein Status-Bot-Limit ist erreicht. Hol dir über Get more zusätzliche Gratis-Bots oder wähle Premium.','Your status-bot limit has been reached. Get more free bots through Get more or choose Premium.')); return res.redirect('/'); }
...
app.post('/servers/new', requireLogin, checkCsrf, async (req, res) => {
  ...
  } catch (error) { flash(req, /limit/i.test(String(error.message)) ? 'limit' : 'err', error.message); res.redirect('/servers/new'); }
});
