...
app.post('/bot-services/:id/manage', requireLogin, checkCsrf, async(req,res)=>{
  ...
  try{
    ...
    appendManagedAudit(bot.id,{actor:`web:${user.discordId}`,action:'config-saved',detail:`Dynamic Ban: ${dynamicBanEnabled?'on':'off'} · Ban Sync Community: ${bot.banSyncServerId||'off'}`});
    await syncAllServiceBots();
