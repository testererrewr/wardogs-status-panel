...
app.get('/bot-services', (req, res) => {
  ...
  render(req,res,tr(lang,'Bot Services','Bot Services'),`<div class="pagehead"><div><h1>${tr(lang,'Bots as a Service','Bots as a Service')}</h1><p>${tr(lang,'Einzelne, von status-hub.lol betriebene Spezial-Bots. Derselbe Service kann mehrfach gebucht werden; jede Instanz hat ihr eigenes Abo und ihre eigene Konfiguration.','Individual specialist bots operated by status-hub.lol. The same service can be purchased multiple times; every instance has its own subscription and configuration.')}</p></div></div>${categoryFilter}<section class="service-grid">${cards || `<div class="panel empty">${tr(lang,'Keine Bot Services in dieser Kategorie.','No bot services in this category.')}</div>`}</section><div class="panel help service-note"><strong>${tr(lang,'Wichtig','Important')}:</strong> ${tr(lang,'Jede Managed-Bot-Instanz hat ihr eigenes Abo. Premium-Status-Bots und Custom-Bot-Freigaben bleiben davon getrennt.','Each managed-bot instance has its own subscription. Premium status bots and custom-bot permissions remain separate.')}</div>`);
});
...
function managedBotForm(req, service, bot) {
  ...
  const grantRows=grants.map((row,i)=>managedGrantRowHtml(req,row,i)).join('');
   return `<form method="post" action="/bot-services/${encodeURIComponent(service.id)}/manage" class="panel formgrid" id="managed-bot-config">
    ...
    <label>Discord Bot Token (${tr(lang,'optional','optional')})<input name="botToken" type="password" autocomplete="new-password" placeholder="${bot.botTokenEnc?tr(lang,'Leer lassen = unverändert','Leave blank = unchanged'):tr(lang,'Nur für Discord Top 25 nötig','Only needed for Discord Top 25')}"><span class="muted small">${tr(lang,'Nur erforderlich, wenn ein Discord Top-25 Channel gesetzt ist.','Only required when a Discord Top 25 channel is configured.')}</span></label>
    ...
    <label class="check span2"><input type="checkbox" name="controlPanelEnabled" value="1" ${bot.controlPanelEnabled===true?'checked':''}> <strong>${tr(lang,'Discord Management Panel aktivieren','Enable Discord management panel')}</strong> · ${tr(lang,'Das Panel wird automatisch wieder als letzte Nachricht im gewählten Channel platziert.','The panel is automatically moved back to the bottom of the selected channel.')}</label>
    <label class="span2">Discord Management Panel Channel ID<input name="controlPanelChannelId" inputmode="numeric" value="${esc(bot.controlPanelChannelId||'')}" placeholder="123456789012345678"><span class="muted small">${tr(lang,'Kann derselbe Channel wie der Alert-Channel sein.','May be the same channel as the alert channel.')}</span></label>
    <div class="span2 managed-config-block"><div class="row between"><div><strong>${tr(lang,'Discord Rechtevergabe','Discord permission grants')}</strong><div class="muted small">${tr(lang,'Rolle oder Benutzer-ID eintragen und exakt anhaken, was erlaubt ist. Bot-Owner und Discord-Administratoren dürfen immer alles.','Enter a role or user ID and tick exactly what it may do. The bot owner and Discord administrators always have full access.')}</div></div><button class="button ghost smallbtn" type="button" id="add-managed-grant">+ ${tr(lang,'Freigabe','Grant')}</button></div><div id="managed-grants" class="managed-grants" data-lang="${esc(lang)}" data-next-index="${grants.length}">${grantRows}</div></div>
    ...
    <label class="check span2"><input type="checkbox" name="announcementEnabled" value="1" ${bot.announcementEnabled===true?'checked':''}> <strong>${tr(lang,'Automatische Server-Announcements aktivieren','Enable scheduled server announcements')}</strong> · ${tr(lang,'Standard ist AUS.','Default is OFF.')}</label>
    ...
    <div class="span2 managed-config-block"><div class="row between"><div><strong>${tr(lang,'Steam Detection Rules','Steam detection rules')}</strong><div class="muted small">${tr(lang,'VAC-/Game-Bans, geringe Spielzeit, neue Konten und weitere Steam-Risiken prüfen. Leere Regeln gibt es nicht mehr – ohne Regel kannst du andere Panel-Einstellungen normal speichern.','Check VAC/game bans, low playtime, new accounts and other Steam risk signals. There is no blank default rule anymore, so other panel settings can be saved without configuring a rule.')}</div></div><button class="button ghost smallbtn" type="button" id="add-managed-rule">+ ${tr(lang,'Regel','Rule')}</button></div><div class="managed-steam-settings"><label>${tr(lang,'Steam Web API Key','Steam Web API key')}<input name="steamWebApiKey" type="password" autocomplete="new-password" placeholder="${bot.steamWebApiKeyEnc?tr(lang,'Leer lassen = unverändert','Leave blank = unchanged'):hasGlobalSteamKey?tr(lang,'Globaler Key ist konfiguriert','Global key is configured'):tr(lang,'Für Steam-Regeln erforderlich','Required for Steam rules')}"><span class="muted small">${tr(lang,'Optional pro Bot. Alternativ kann der Host STEAM_WEB_API_KEY global setzen.','Optional per bot. The host can alternatively set STEAM_WEB_API_KEY globally.')}</span></label><div class="managed-auto-steam"><strong>${tr(lang,'Automatische Steam-Erkennung','Automatic Steam detection')}</strong><span class="muted small">${tr(lang,'WARDOGS-Spiel und Steam64IDs werden automatisch erkannt. Du musst keine App ID oder Spieler-ID eintragen.','The WARDOGS game and player Steam64 IDs are detected automatically. You do not need to enter an App ID or player ID.')}</span></div></div><div id="managed-rules" class="managed-rules" data-lang="${esc(lang)}" data-next-index="${rules.length}">${ruleRows||`<div class="muted small managed-rule-empty">${tr(lang,'Noch keine Detection Rule aktiv.','No detection rule active yet.')}</div>`}</div></div>
...
function playtimeTrackerForm(req,service,bot) {
  ...
  return `<form method="post" action="/bot-services/${encodeURIComponent(service.id)}/manage" class="panel formgrid" id="playtime-tracker-config">
    ...
    <label>${tr(lang,'Statistik-Zeitzone','Statistics timezone')}<input name="statsTimezone" maxlength="80" value="${esc(bot.statsTimezone||'Europe/Vienna')}" placeholder="Europe/Vienna"><span class="muted small">IANA timezone</span></label>
    <label class="span2">Discord Top-25 Channel ID (${tr(lang,'optional','optional')})<input name="leaderboardChannelId" inputmode="numeric" value="${esc(bot.leaderboardChannelId||'')}" placeholder="123456789012345678"><span class="muted small">${tr(lang,'Wenn gesetzt, wird eine einzelne Top-25 Nachricht alle 6 Stunden aktualisiert statt den Channel vollzuspammen.','When set, one persistent Top 25 message is updated every 6 hours instead of spamming the channel.')}</span></label>
    ...
    ${u.role==='admin'?`<label class="check span2"><input type="checkbox" name="allowPrivateTarget" value="1" ${bot.allowPrivateTarget?'checked':''}> ${tr(lang,'Private/LAN WARDOGS-Ziele erlauben (Admin)','Allow private/LAN WARDOGS targets (admin)')}</label>`:''}
    <div class="span2 help"><strong>${tr(lang,'Tracking','Tracking')}:</strong> ${tr(lang,'Die Steam64ID wird automatisch aus /v1/players gelesen. Spielzeit wird nur gezählt, während der Spieler auf genau diesem WARDOGS-Server online ist.','Steam64ID is read automatically from /v1/players. Playtime is counted only while the player is online on this exact WARDOGS server.')}</div>
    <div class="span2 actions wrap"><button class="button primary" type="submit">${tr(lang,'Speichern','Save')}</button><button class="button ghost" type="submit" formaction="/bot-services/${encodeURIComponent(service.id)}/test">${tr(lang,'Verbindung testen','Test connection')}</button>${bot.enabled?`<button class="button ghost" type="submit" formaction="/managed-bots/${esc(bot.id)}/restart">${tr(lang,'Neu starten','Restart')}</button><button class="button danger" type="submit" formaction="/managed-bots/${esc(bot.id)}/stop">Stop</button>`:`<button class="button success" type="submit" formaction="/managed-bots/${esc(bot.id)}/restart">${tr(lang,'Starten','Start')}</button>`}<span class="badge ${rt.state==='online'?'online':rt.state==='error'?'error':'neutral'}">${esc(rt.state||'stopped')}</span></div>
...
function playtimeStatsPanel(req,bot) {
  ...
  const hourRows=snap.hours.slice(0,12).map((h)=>`<tr><td>${String(h.hour).padStart(2,'0')}:00–${String((h.hour+1)%24).padStart(2,'0')}:00</td><td>${h.samples?Number(h.averagePlayers).toFixed(1):'—'}</td><td>${esc(h.maxPlayers)}</td></tr>`).join('');
...
function managedOperationsPanel(req, bot, live) {
  ...
  return `<div class="managed-live">
    <div class="pagehead compact managed-live-head"><div><h2>${l(req,'Live Server Management','Live server management')}</h2><p>${l(req,'Direkte RCON-Steuerung ohne den Discord-Bot neu zu starten.','Direct RCON control without restarting the Discord bot.')}</p></div><a class="button ghost" href="${esc(managedManageUrl(bot.serviceId,bot.id))}">${l(req,'Aktualisieren','Refresh')}</a></div>
...
app.post('/bot-services/:id/admin-activate', requireAdmin, checkCsrf, (req,res)=>{
  ...
  const bot=upsertManagedBot({ownerDiscordId:user.discordId,serviceId:service.id,name:`${service.nameDe||service.nameEn||'WARDOGS Bot'} #${existing.length+1}`,enabled:false,autoBanEnabled:false,pollSeconds:service.id===PLAYTIME_SERVICE_ID?30:20,rulesText:'',statsTimezone:'Europe/Vienna',adminGrant:true,accessSource:'admin',accessRecordId:'',accessUntil:null});
  rebalanceAssignments();
...
app.get('/bot-services/:id/manage', requireLogin, async(req,res)=>{
  ...
  if(!selected){
    return render(req,res,service.nameDe||service.nameEn,`<div class="pagehead"><div><h1>${esc(service.nameDe||service.nameEn)}</h1><p>${l(req,'Du kannst denselben Bot-Service mehrfach als getrennte Instanzen buchen.','You can purchase the same bot service multiple times as separate instances.')}</p></div><a class="button ghost" href="/bot-services">${l(req,'Zurück','Back')}</a></div>${overview}`);
  }
...
app.post('/managed-bots/:id/lighting',requireLogin,managedActionRate,checkCsrf,async(req,res)=>{const bot=managedActionContext(req,res);if(!bot)return;try{await setManagedLighting(bot,String(req.body.lighting||''));flash(req,'ok',l(req,'Lighting wurde geändert.','Lighting changed.'));}catch(error){flash(req,'err',error.message);}return managedBack(res,bot);});
