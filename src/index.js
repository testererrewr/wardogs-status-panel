...
import { managedBotRuntime, parseManagedRules, testManagedWardogs, syncManagedBots, restartManagedBot, stopManagedBot, shutdownManagedBots, managedDashboard, broadcastManaged, banManagedPlayer, kickManagedPlayer, killManagedPlayer, whisperManagedPlayer, moveManagedPlayer, unbanManagedPlayer, addManagedReservedSlot, removeManagedReservedSlot, restartManagedMatch, endManagedMatch, setManagedLighting, changeManagedMap } from './managed-bots.js';
...
function managedOperationsPanel(req, bot, live) {
  ...
  const playerRows=players.map((p)=>{
    const steam=String(p.steamId||p.steamId64||''),valid=/^\d{17}$/.test(steam),name=String(p.name||'Unknown'),faction=String(p.faction||'—');
    const actionBase=valid?`<div class="managed-player-actions">
      ...
      ${supports('PATCH','/v1/players/{steamId}')?`<form method="post" action="/managed-bots/${esc(bot.id)}/player/faction"><input type="hidden" name="_csrf" value="${token}"><input type="hidden" name="steamId" value="${esc(steam)}"><input name="faction" maxlength="80" required value="${esc(faction==='—'?'':faction)}" placeholder="Faction"><button class="button ghost smallbtn">${l(req,'Team setzen','Set team')}</button></form>`:''}
  ...
  const mapOptions=mapRows.map((m)=>`<option value="${esc(m.id||m.name||'')}">${esc(m.displayName||m.id||m.name||'')}</option>`).join('');
  const lightingOptions=lightingRows.map((x)=>`<option value="${esc(x.id||x.name||'')}">${esc(x.displayName||x.id||x.name||'')}</option>`).join('');
  const expHint=expRows.slice(0,20).map((x)=>String(x.id||x.name||'')).filter(Boolean).join(', ');
  ...
  return `<div class="managed-live">
    ...
    <section class="managed-grid">
      ...
      ${mapOptions?`<form method="post" action="/managed-bots/${esc(bot.id)}/match/map" class="panel formgrid managed-section"><div class="span2 managed-heading"><div><span class="eyebrow">Map</span><h2>${l(req,'Map wechseln','Change map')}</h2></div></div><input type="hidden" name="_csrf" value="${token}"><label>Map<select name="map" required>${mapOptions}</select></label><label>Lighting<select name="lighting"><option value="">${l(req,'Server-Standard','Server default')}</option>${lightingOptions}</select></label><label class="span2">Experiences <input name="experiences" maxlength="500" placeholder="${esc(expHint)}"><span class="muted small">${l(req,'Optional, mit Kommas trennen.','Optional, comma separated.')}</span></label><label class="span2">Zone Alternator<input name="zoneAlternator" maxlength="160" placeholder="ZoneAlternator…"></label><div class="span2 actions"><button class="button primary">${l(req,'Mapwechsel senden','Send map change')}</button></div></form>`:''}
...
app.post('/bot-services/:id/manage', requireLogin, checkCsrf, async(req,res)=>{
  ...
  try{
    ...
    const patch={id:bot.id,name,alertChannelId,mentionRoleId,wardogsBaseUrl,pollSeconds:Math.max(10,Math.min(300,Number(req.body.pollSeconds)||20)),rulesText,autoBanEnabled:req.body.autoBanEnabled==='1',announcementEnabled:req.body.announcementEnabled==='1',announcementIntervalMinutes:Math.max(1,Math.min(1440,Number(req.body.announcementIntervalMinutes)||15)),announcementMessages:announcementMessages.join('\n'),enabled:req.body.enabled==='1',allowPrivateTarget:user.role==='admin'?req.body.allowPrivateTarget==='1':Boolean(bot.allowPrivateTarget),restartNonce:Date.now()};
...
app.post('/managed-bots/:id/match/map',requireLogin,managedActionRate,checkCsrf,async(req,res)=>{const bot=managedActionContext(req,res);if(!bot)return;try{await changeManagedMap(bot,{map:String(req.body.map||''),experiences:String(req.body.experiences||'').split(',').map((x)=>x.trim()).filter(Boolean),lighting:String(req.body.lighting||''),zoneAlternator:String(req.body.zoneAlternator||'')});flash(req,'ok',l(req,'Mapwechsel wurde gesendet.','Map change sent.'));}catch(error){flash(req,'err',error.message);}return managedBack(res,bot);});
