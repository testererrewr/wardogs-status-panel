...
function playtimeTrackerForm(req,service,bot) {
  ...
  return `<form method="post" action="/bot-services/${encodeURIComponent(service.id)}/manage" class="panel formgrid" id="playtime-tracker-config">
    ...
    <div class="span2 help"><strong>WARDOGS Status Bot:</strong> ${tr(lang,'Jeder eingetragene Gameserver hat seinen eigenen festen Discord-Killfeed mit den letzten 15 Kills. Discord↔Game-Verknüpfung per /linkgame, /status, /whois, /whoisplayer und /playerstats laufen über denselben Bot. Das Discord-Leaderboard zeigt Top 15 pro Kategorie und eine Spielersuche mit exakter Platzierung.','Every configured game server has its own fixed Discord killfeed with the latest 15 kills. Discord↔game linking via /linkgame, /status, /whois, /whoisplayer and /playerstats run through the same bot. The Discord leaderboard shows the Top 15 per category plus player search with exact ranking.')}</div>
    ...
    ${!bot.botTokenEnc?`<div class="span2 warning"><strong>Discord:</strong> ${tr(lang,'Für /linkgame, /status, /whois, /whoisplayer, /playerstats und das Discord-Leaderboard einen Discord Bot Token hinterlegen.','Add a Discord Bot Token for /linkgame, /status, /whois, /whoisplayer, /playerstats and the Discord leaderboard.')}</div>`:''}${rt.lastError?`<div class="span2 warning"><strong>Runtime:</strong> ${esc(rt.lastError)}</div>`:''}${rt.lastLeaderboardError?`<div class="span2 warning"><strong>Discord Leaderboard:</strong> ${esc(rt.lastLeaderboardError)}</div>`:''}${rt.slashCommandError?`<div class="span2 warning"><strong>Discord Slash Commands:</strong> ${esc(rt.slashCommandError)}</div>`:''}
  </form><section class="panel managed-config-block"><div class="row between"><strong>${tr(lang,'Config Export / Import','Config export / import')}</strong><a class="button ghost smallbtn" href="/managed-bots/${esc(bot.id)}/config/export">${tr(lang,'Config exportieren','Export config')}</a></div><form method="post" enctype="multipart/form-data" action="/managed-bots/${esc(bot.id)}/config/import" class="managed-add-row"><input type="hidden" name="_csrf" value="${esc(csrf(req))}"><input type="file" name="config" accept="application/json,.json" required><button class="button ghost smallbtn">${tr(lang,'Config importieren','Import config')}</button></form></section>
...
function playtimeStatsPanel(req,bot) {
  ...
  const topRows=snap.top25.slice(0,15).map((p,i)=>`<tr><td>${i+1}</td><td><strong>${esc(p.name)}</strong><div class="muted small"><a href="https://steamcommunity.com/profiles/${esc(p.steamId)}" target="_blank" rel="noopener">${esc(p.steamId)}</a>${p.online?' · online':''}</div></td><td>${esc(p.clanTag||'—')}</td><td><strong>${esc(trackedDuration(p.totalSeconds))}</strong></td></tr>`).join('');
  ...
  const killTopRows=killSnap.top25.slice(0,15).map((p,i)=>`<tr><td>${i+1}</td><td><strong>${esc(p.name||p.steamId)}</strong><div class="muted small">${esc(p.steamId)}</div></td><td>${esc(p.kills)}</td><td>${esc(p.deaths)}</td><td>${esc(Number(p.kd||0).toFixed(2))}</td><td>${esc(p.headshots)}</td><td>${esc(Number(p.longestKillMeters||0).toFixed(1))} m</td></tr>`).join('');
