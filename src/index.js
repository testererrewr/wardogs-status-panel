...
import {
  ...
  getManagedBot, getManagedBotForUserService, getManagedBotByAccessRecord, listManagedBotsForUserService, listManagedBotsFor, upsertManagedBot, deleteManagedBot, listBanSyncServers, getBanSyncServer,
...
function managedBotForm(req, service, bot) {
  ...
  <script src="/managed.js?v=3.12.35" defer></script>`;
}
...
function playtimeStatsPanel(req,bot) {
  ...
  const killSnap=killStatsSnapshotFromStats(bot.killStats);
  ...
  const killMatches=query?searchKillStatsSnapshot(killSnap,query):[];
  const killSearchRows=killMatches.map((p)=>`<tr><td><strong>${esc(p.name||p.steamId)}</strong><div class="muted small">${esc(p.steamId)}</div></td><td>${esc(p.kills)}</td><td>${esc(p.deaths)}</td><td>${esc(Number(p.kd||0).toFixed(2))}</td><td>${esc(p.headshots)}</td><td>${esc(Number(p.longestKillMeters||0).toFixed(1))} m</td><td>${esc((p.topCauses||[]).slice(0,3).map(([cause,count])=>`${prettyCause(cause)}: ${count}`).join(' · ')||'—')}</td></tr>`).join('');
  const killTopRows=killSnap.top25.map((p,i)=>`<tr><td>${i+1}</td><td><strong>${esc(p.name||p.steamId)}</strong><div class="muted small">${esc(p.steamId)}</div></td><td>${esc(p.kills)}</td><td>${esc(p.deaths)}</td><td>${esc(Number(p.kd||0).toFixed(2))}</td><td>${esc(p.headshots)}</td><td>${esc(Number(p.longestKillMeters||0).toFixed(1))} m</td></tr>`).join('');
  ...
  return `<section class="playtime-dashboard">
    ...
    <div class="panel tablewrap"><div class="managed-heading padded"><div><span class="eyebrow">${l(req,'Globale Spielersuche','Global player search')}</span><h2>${l(req,'Name, Alias oder Steam64ID','Name, alias or Steam64ID')}</h2></div></div><form method="get" action="/bot-services/${encodeURIComponent(bot.serviceId)}/manage" class="managed-add-row"><input type="hidden" name="bot" value="${esc(bot.id)}"><input name="playerq" maxlength="100" value="${esc(query)}" placeholder="${l(req,'Name, Alias oder Steam64ID','Name, alias or Steam64ID')}"><button class="button primary smallbtn">${l(req,'Suchen','Search')}</button></form>${query?`<h3>${l(req,'Playtime','Playtime')}</h3><table><thead><tr><th>${l(req,'Spieler','Player')}</th><th>${l(req,'Gesamtspielzeit','Total playtime')}</th><th>Sessions</th><th>${l(req,'Letzte Aktivität','Last activity')}</th><th>${l(req,'Server','Servers')}</th></tr></thead><tbody>${searchRows||`<tr><td colspan="5">${l(req,'Keine Playtime-Daten gefunden.','No playtime data found.')}</td></tr>`}</tbody></table><h3>${l(req,'Globale Kill-Stats','Global kill stats')}</h3><table><thead><tr><th>${l(req,'Spieler','Player')}</th><th>Kills</th><th>${l(req,'Tode','Deaths')}</th><th>K/D</th><th>Headshots</th><th>${l(req,'Längster Kill','Longest kill')}</th><th>${l(req,'Top Ursachen','Top causes')}</th></tr></thead><tbody>${killSearchRows||`<tr><td colspan="7">${l(req,'Keine Kill-Stats gefunden.','No kill stats found.')}</td></tr>`}</tbody></table>`:''}</div>
    ...
    <div class="panel tablewrap"><div class="managed-heading padded"><div><span class="eyebrow">${l(req,'Globale Kill-Stats','Global kill stats')}</span><h2>${l(req,'Top 25 Kills · alle eingebundenen Server','Top 25 kills · all connected servers')}</h2></div></div><table><thead><tr><th>#</th><th>${l(req,'Spieler','Player')}</th><th>Kills</th><th>${l(req,'Tode','Deaths')}</th><th>K/D</th><th>Headshots</th><th>${l(req,'Längster Kill','Longest kill')}</th></tr></thead><tbody>${killTopRows||`<tr><td colspan="7">${l(req,'Noch keine Kill-Stats erfasst.','No kill stats tracked yet.')}</td></tr>`}</tbody></table></div>
...
app.get('/bot-services/:id/manage', requireLogin, async(req,res)=>{
  ...
  if(service.id===PLAYTIME_SERVICE_ID){
    ...
    return render(req,res,service.nameDe||service.nameEn,`<div class="pagehead"><div><h1>${esc(selected.name||service.nameDe||service.nameEn)}</h1></div><a class="button ghost" href="/bot-services">${l(req,'Zurück','Back')}</a></div>${overview}<section class="account-grid">${subHtml}<article class="panel"><span class="eyebrow">WARDOGS Status Bot</span><h2>${esc(rt.state||'stopped')}</h2><p>${l(req,'Mehrere Server, Playtime, eigener Killfeed je Server und globale Kill-Stats.','Multiple servers, playtime, one killfeed per server and global kill stats.')}</p></article></section>${playtimeTrackerForm(req,service,selected)}${playtimeStatsPanel(req,selected)}${deleteHtml}`);
  }
...
app.post('/bot-services/:id/manage', requireLogin, checkCsrf, async(req,res)=>{
  ...
  try{
    ...
    const secret=String(req.body.wardogsSecret||'').trim(); if(secret)patch.wardogsSecretEnc=encryptSecret(secret); else if(!bot.wardogsSecretEnc)throw new Error(l(req,'WARDOGS RCON/API Passwort fehlt.','WARDOGS RCON/API password is required.'));
    const dynamicTargetChanged=String(wardogsBaseUrl||'').replace(/\/+$/,'')!==String(bot.wardogsBaseUrl||'').replace(/\/+$/,'');
    if(dynamicTargetChanged&&bot.dynamicNameOriginalName){try{await restoreManagedDynamicServerName(bot);}catch{}}
    if(dynamicTargetChanged)patch.dynamicNameOriginalName='';
    bot=upsertManagedBot(patch);
    if(!dynamicNameEnabled&&bot.dynamicNameOriginalName){try{await restoreManagedDynamicServerName(bot);bot=getManagedBot(bot.id)||bot;}catch(error){appendManagedAudit(bot.id,{actor:`web:${user.discordId}`,action:'dynamic-name-restore-failed',detail:String(error?.message||error).slice(0,300),status:'error'});}}
    if(isCommunityMember&&isCommunityOwner){
