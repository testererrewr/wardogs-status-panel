...
function mode() { return String(process.env.PAYPAL_MODE || 'sandbox').toLowerCase() === 'live' ? 'live' : 'sandbox'; }
function baseUrl() { return process.env.PAYPAL_API_BASE || (mode() === 'live' ? 'https://api-m.paypal.com' : 'https://api-m.sandbox.paypal.com'); }
export function paypalEnvironment() { return mode(); }
export function paypalConfigured() { return Boolean(process.env.PAYPAL_CLIENT_ID && process.env.PAYPAL_CLIENT_SECRET); }

async function accessToken() {
  if (!paypalConfigured()) throw new Error('PayPal API credentials are not configured');
...
export async function createCheckoutOrder({ purchaseId, planId, amount, currency, returnUrl, cancelUrl }) {
  const data = await api('/v2/checkout/orders', {
    ...
    body: {
      ...
      purchase_units: [{
        ...
        description: `status-hub.lol ${planId}`,
        custom_id: purchaseId,
        invoice_id: `SH-${purchaseId}`.slice(0, 127),
        amount: { currency_code: currency, value: amount }
      }],
      payment_source: {
        paypal: {
          experience_context: {
            brand_name: 'status-hub.lol',
...
export function extractCompletedCapture(order) {
  ...
  return {
    ...
    amount: capture?.amount?.value || pu?.amount?.value || '',
...
export async function verifyWebhook(headers, event, webhookId) {
  ...
  const result = await api('/v1/notifications/verify-webhook-signature', { method: 'POST', body: payload });
  return result.verification_status === 'SUCCESS';
}

export async function ensureWebhook(existingId, url) {
  if (existingId) {
    try {
      const current = await api(`/v1/notifications/webhooks/${encodeURIComponent(existingId)}`);
